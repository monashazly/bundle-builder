...
## Session 8 — README + polish

**Branch:** `chore/final`
**Depends on:** feat/layout merged

```
Read CLAUDE.md.

1. Generate placeholder product images:
   Create a script scripts/generate-placeholders.mjs that uses the `canvas`
   npm package (or just write static SVGs) to create simple colored rectangles
   for each product in public/images/. Each SVG should show the product name
   centered in a rounded rectangle with a category-appropriate color.

   Alternatively: create public/images/*.svg files manually as simple
   colored rectangles with the product name as a text label.

2. Write README.md with exactly these sections:
   - ## Overview (one paragraph)
   - ## Run instructions (npm install, npm run dev, npm run test, npm run build)
   - ## Architecture
     - Framework choice and why
     - State management approach
     - Data layer isolation (lib/api.ts)
     - Shared types across frontend and Route Handlers
   - ## API endpoints (table: method, path, description)
   - ## Decisions & tradeoffs (honest paragraph per decision)
   - ## What I didn't finish (honest list)

3. Final build checklist:
   npm run typecheck — must be clean (0 errors)
   npm run lint — must be clean
   npm run test — all tests pass (list count)
   npm run build — must succeed

Commit: chore: add README, image placeholders, final build verification
Push chore/final. Open PR.
```
